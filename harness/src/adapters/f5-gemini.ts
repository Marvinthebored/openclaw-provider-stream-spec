  ...
  normalize(frames) {
    ...
    for (const frame of frames) {
      if (frame.comment || frame.done) {
        ...
        // TODO: implement SPEC section 6 F5 Gemini mapping, including thoughtSignature transcript capture.
    ...
    ctx.emit(undefined, "lifecycle", {
      ...
      reason: "error",
      status: "incomplete",
      diagnostic: { code: "adapter_stub", detail: "F5 Gemini adapter not implemented" }
    });
    return ctx.result();
  }
};
